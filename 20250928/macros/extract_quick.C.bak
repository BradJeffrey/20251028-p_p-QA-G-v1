...
static double ks_uniform_p(TH1* h) {
  if(!h || h->GetEntries()<=0) return TMath::QuietNaN();
  // Build a flat histogram with same binning and total entries
  ...
  // Use Kolmogorov test (returns p-value)
  double p = h->KolmogorovTest(ref, "M"); // "M" to use Mises or default?
  delete ref;
  return p;
}
static double chi2_uniform_red(TH1* h) {
  ...
  for(int i=1;i<=nb;++i) {
    ...
    double var = std::max(1.0, expect); // avoid zero variance
    chi2 += (obs - expect)*(obs - expect)/var;
    ++dof;
  }
  // reduce by dof-1 (one constraint: sum fixed)
...
void extract_quick(const char* list="lists/files.txt") {
  ...
  // Open per-metric CSVs (append if re-run)
  ...
  auto write_header = [](std::ofstream& f){
    f << "run,segment,file,value,error,weight\n";
  };
  ...
  while(std::getline(in, fpath)) {
    ...
    // crude run parsing: try to catch runNNNNN in filename
    long run = 0; {
      // find "run" then digits
      size_t p = fpath.find("run");
      if(p != std::string::npos) {
        size_t i = p+3; while(i<fpath.size() && isdigit(fpath[i])) { run = run*10 + (fpath[i]-'0'); ++i; }
      }
    }
    int seg = -1; // unknown (not used further)
...
// =======================================================================
// [ADDED] Extra metrics: cluster size mean, cluster phi RMS, hit asymmetry
// =======================================================================

// Open additional CSV outputs
...
auto write_header_extra = [](std::ofstream& f){ f << "run,segment,file,value,error,weight\n"; };
write_header_extra(csv_cluster_size_mean);
write_header_extra(csv_cluster_phi_rms);
write_header_extra(csv_hits_asym);
