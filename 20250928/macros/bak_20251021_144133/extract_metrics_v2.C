...
static FileMeta parse_meta(const std::string& path) {
  ...
  if (std::regex_search(m.base, sm, r_run)) {
    ...
    std::string s=m.base;
    while (std::regex_search(s, sm, r_digits)) { m.run = std::stoi(sm[1]); s = sm.suffix().str(); }
  }
  ...
  if (std::regex_search(m.base, sm, r_seg_suffix)) {
    ...
    if (std::regex_search(m.base, sm, r_seg_tag)) m.segment = std::stoi(sm[1]);
  }
  return m;
}
...
struct MetricDef {
  ...
  std::string method;               // mean|rms|sum|maxbin|gauspeak|asym|quantilep|truncmean|mean_win|uniform_r1|chi2_uniform
...
static double hist_counts(TH1* h){ return h->Integral(1, h->GetNbinsX()); }

static double hist_quantile(TH1* h, double p) {
  p = clamp01(p);
  const int nb = h->GetNbinsX();
  double tot = hist_counts(h);
  if (tot <= 0) return std::numeric_limits<double>::quiet_NaN();
  const double target = p * tot;
  double acc = 0.0;
  for (int i=1;i<=nb;i++){
    acc += h->GetBinContent(i);
    if (acc >= target) return h->GetXaxis()->GetBinCenter(i);
  }
  return h->GetXaxis()->GetBinCenter(nb);
}
static std::pair<double,double> trunc_window_by_quantiles(TH1* h, double qlo, double qhi) {
  double xlo = hist_quantile(h, clamp01(qlo));
  double xhi = hist_quantile(h, clamp01(qhi));
  if (xhi < xlo) std::swap(xlo, xhi);
  return {xlo, xhi};
}
static double mean_in_window(TH1* h, double xlo, double xhi, double& used_counts) {
...
static double chi2_uniform_reduced(TH1* h) {
  ...
  double tot = hist_counts(h);
  if (tot<=0) return std::numeric_limits<double>::quiet_NaN();
  double exp = tot / nb; if (exp<=0) return std::numeric_limits<double>::quiet_NaN();
  double chi2=0;
  for (int i=1;i<=nb;i++){
    double o = h->GetBinContent(i);
    chi2 += (o-exp)*(o-exp)/exp;
  }
  double dof = nb - 1.0; return chi2/dof;
}

// value+error+weight (counts used)
struct MVal { double y, ey, w; };

static MVal make_val(double y, double ey, double w){
  MVal mv{y,ey,w};
  if (!std::isfinite(mv.w) || mv.w<0) mv.w=0;
  return mv;
}

static MVal compute_metric(const MetricDef& d, TFile* f) {
  auto getH = [&](const std::string& n)->TH1*{ return (TH1*)f->Get(n.c_str()); };

  if (d.method=="asym") {
    if (d.hnames.size()!=2) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);
    TH1* a=getH(d.hnames[0]); TH1* b=getH(d.hnames[1]);
    if (!a || !b) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);
    double A = d.hasRange? mean_in_window(a,d.xlow,d.xhigh,*(new double())) : hist_counts(a);
    double B = d.hasRange? mean_in_window(b,d.xlow,d.xhigh,*(new double())) : hist_counts(b);
    // If mean_in_window was used above, A/B hold means; for asymmetry we actually want integrals.
    if (d.hasRange) { // recompute integrals in window
      int ba1=a->GetXaxis()->FindBin(d.xlow), ba2=a->GetXaxis()->FindBin(d.xhigh);
      ba1=std::max(1,ba1); ba2=std::min(a->GetNbinsX(),ba2);
      A= a->Integral(ba1,ba2);
      int bb1=b->GetXaxis()->FindBin(d.xlow), bb2=b->GetXaxis()->FindBin(d.xhigh);
      bb1=std::max(1,bb1); bb2=std::min(b->GetNbinsX(),bb2);
      B= b->Integral(bb1,bb2);
    }
    double S=A+B; if (S<=0) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);
    double y=(A-B)/S;
    double ey = std::sqrt(std::max(0.0, 1.0 - y*y) / std::max(1.0, S));
    return make_val(y,ey,S);
  }

  if (d.hnames.empty()) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);
  TH1* h = getH(d.hnames[0]); if (!h) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);
  double N = hist_counts(h);
  if (h->GetEntries()==0 && N==0) return make_val(std::numeric_limits<double>::quiet_NaN(),0,0);

  if (d.method=="mean") {
    double y=h->GetMean(); double rms=h->GetRMS();
    double ey = (h->GetEntries()>0 && std::isfinite(rms)) ? rms/std::sqrt(h->GetEntries()) : 0.0;
    return make_val(y,ey,N);
  } else if (d.method=="rms") {
    return make_val(h->GetRMS(), 0.0, N);
  } else if (d.method=="sum") {
    double y = h->Integral(1,h->GetNbinsX());
    double ey = std::sqrt(std::max(0.0,y));
    return make_val(y,ey,y); // weight not used for sum
  } else if (d.method=="maxbin") {
    int ib = h->GetMaximumBin();
    double y = h->GetXaxis()->GetBinCenter(ib);
    return make_val(y,0.0,N);
  } else if (d.method=="gauspeak") {
    double xl = d.hasRange? d.xlow : h->GetXaxis()->GetXmin();
    double xh = d.hasRange? d.xhigh: h->GetXaxis()->GetXmax();
    // count weight in window:
    int b1 = h->GetXaxis()->FindBin(xl), b2=h->GetXaxis()->FindBin(xh);
    b1=std::max(1,b1); b2=std::min(h->GetNbinsX(),b2);
    double w= h->Integral(b1,b2);
    TF1 g("g","gaus", xl, xh);
    auto fr = h->Fit(&g,"QS0");
    if (fr != 0) return make_val(std::numeric_limits<double>::quiet_NaN(),0.0,w);
    double mu = g.GetParameter(1);
    double emu= g.GetParError(1);
    return make_val(mu, emu, w);
    ...
    auto rng = trunc_window_by_quantiles(h, qlo, qhi);
    double used=0; double y = mean_in_window(h, rng.first, rng.second, used);
    return make_val(y, 0.0, used);
  } else if (d.method=="mean_win") {
    if (!d.hasRange) return make_val(std::numeric_limits<double>::quiet_NaN(),0.0,0.0);
    double used=0; double y = mean_in_window(h, d.xlow, d.xhigh, used);
    return make_val(y, 0.0, used);
  } else if (d.method=="uniform_r1") {
    auto pr = phi_uniform_r1(h);
    return make_val(pr.first, pr.second, N);
  } else if (d.method=="chi2_uniform") {
    double y = chi2_uniform_reduced(h);
    return make_val(y, 0.0, N);
  }

  return make_val(std::numeric_limits<double>::quiet_NaN(),0.0,0.0);
}

void extract_metrics_v2(const char* filelist="lists/files.txt",
                        const char* conf="metrics.conf")
{
  ...
  for (auto& d: defs) {
    ...
    outs[d.name].csv<<"run,segment,file,value,error,weight\n";
  ...
  for (auto& tup: files) {
    ...
    for (auto& d: defs) {
      auto mv = compute_metric(d, f.get());
      outs[d.name].csv << run << "," << seg << "," << fpath << "," << mv.y << "," << mv.ey << "," << mv.w << "\n";
      int n = outs[d.name].gr->GetN();
      outs[d.name].gr->SetPoint(n, run, mv.y);
      outs[d.name].gr->SetPointError(n, 0.0, mv.ey);
    }
  }
  ...
  std::cout<<"[DONE] v2 metrics written to out/, plots saved. CSVs now include weight.\n";
}
